...
- multi-tab profile safety (R1, 2026-09-06): every change is a `StoreOperation` (`upsert` / `delete` / `activate` / `import`) committed through one write path that re-reads the latest store, checks the revision the editor started from, applies the operation and bumps `revision`; the read-modify-write runs inside a Web Lock (`navigator.locks`) when available and otherwise reports `atomic: false` instead of claiming exclusion; stale saves of the same profile come back as a conflict with the latest copy (查看最新 / 保留草稿并核对后重存 / 作为新档案保存), imports planned against an older store are re-checked, and `storage` events refresh the other tab's view without touching an open draft; `revision` fields were added to the v5 store and profiles without a schema bump (missing → 0);
- search hits and floor navigation (U1, 2026-09-07): `searchTopics` / `searchNotes` in `apps/web/src/forum/model.ts` report *where* a query matched (title / author / tag / body / floor N / curated note) with a short snippet; list rows show the most useful hit as a button that opens the topic loaded to that floor, scrolled into view, marked and keyboard-focused; matches are highlighted through `highlightHtml`, which escapes archive text segment by segment; curated notes are searched by lesson, applicability and failure modes on any tab, with exact topic / note hit counts and empty states; public reading state (`q`, `realm`, `node`, `tab`, `topic`, `reply`) lives in the hash (`parseHashState` / `serializeHashState`, legacy `#topic=<id>` and V3 slugs still resolve, malformed encodings are reported instead of thrown, nothing from a profile is ever written there); topics opened from the list or from a related-thread link push history entries so Back returns list ← A ← B, closing a topic pops those entries, a topic opened from a direct link closes back to the in-site list, and focus returns to the element that opened it;
